  ...
  describe ('Eits', () => {
    describe ('methods', () =>{
      ...
      it('can insert EIT', () => {
        const name = 'Lawrence';
        const age = 21;
        const phone = 0804353567;
        const country = 'Nigeria';
        const insert = Meteor.server.method_handlers['eits.insert'];
        const invocation = { userId };
        insert.apply(invocation, [name, age, phone, country]);
      ...
      it('cannot insert EIT if not logged in', () => {
        ...
        assert.throws(() =>  insert.apply(invocation, [name, age, phone, country]),
      ...
      it ('can edit EIT', () => {
        const name = 'Lawna';
        const age = 25;
        const phone = 08043453567;
        const country = 'Liberia';
        const owner = userId;
        const update = Meteor.server.method_handlers['eits.edit'];
        const invocation = { userId };
        update.apply(invocation, [eitId, name, age, phone, country, owner]);
      ...
      it ('cannot edit EIT if not logged in', () => {
        ...
        const update = Meteor.server.method_handlers['eits.edit'];
        const invocation = {};
        assert.throws(() =>  update.apply(invocation, [eitId, name, age, phone, country, owner]),
          Meteor.Error, '[not-authorized]')
        assert.equal(Eits.find().count(), 1);
      })
